...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONUP:
            if hovering is None and edit is not None:
                Wires.pop(-1)
            elif hovering is not None and hovering.color != Wires[-1].color:
                Wires.pop(-1)
            elif hovering is not None and hovering.color == Wires[-1].color and edit is not None and last != hovering:
